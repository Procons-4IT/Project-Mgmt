...
#Region "Closing Project Activity"
    Public Sub ClosingProjectActivity(ByVal aEmpID As String, ByVal aProjectCode As String, ByVal aModule As String, ByVal aActivity As String, ByVal aDate As Date)
        Dim oRec, oRec1, oRec2 As SAPbobsCOM.Recordset
        Dim dbltotalEstimatedDays, dblTotalEstimatedHours, dblTotalTimesheet As Double
        Dim strProjectQuery, strBudgetQuery, strTimesheetQuery As String
        ...
        strBudgetQuery = "Select Sum(U_Z_Hours) 'U_Z_Hours' from [@Z_PRJ1] T0 Inner Join [@Z_HPRJ] T1 on T1.DocEntry=T0.DocEntry where T1.U_Z_PRJCODE='" & aProjectCode & "' and U_Z_MODNAME='" & aModule & "' and U_Z_ACTNAME='" & aActivity & "' and T0.U_Z_EMPID='" & aEmpID & "'"
        oRec.DoQuery(strBudgetQuery)
        If oRec.RecordCount > 0 Then
            dblTotalEstimatedHours = oRec.Fields.Item("U_Z_Hours").Value
        Else
            dblTotalEstimatedHours = 0
        End If
        strTimesheetQuery = "Select SUM(T1.U_Z_HOURS),T0.U_Z_EMPCODE  from [@Z_OTIM]  T0 inner Join [@Z_TIM1] T1  on T1.U_Z_REFCODE=T0.Code  where T1.U_Z_APPROVED='A' and T0.U_Z_EMPCODE='" & aEmpID & "' and T1.U_Z_PRJCODE='" & aProjectCode & "' and T1.U_Z_PRCNAME='" & aModule & "' and T1.U_Z_ACTNAME ='" & aActivity & "' group by T0.U_Z_EMPCODE "
        oRec1.DoQuery(strTimesheetQuery)
        If oRec1.RecordCount > 0 Then
            dblTotalTimesheet = oRec1.Fields.Item(0).Value
        Else
            dblTotalTimesheet = 0
        End If
        If dblTotalTimesheet >= dblTotalEstimatedHours Then
            strTimesheetQuery = "Select U_Z_DATE  from [@Z_OTIM]  T0 inner Join [@Z_TIM1] T1  on T1.U_Z_REFCODE=T0.Code  where T1.U_Z_APPROVED='A' and T0.U_Z_EMPCODE='" & aEmpID & "' and T1.U_Z_PRJCODE='" & aProjectCode & "' and T1.U_Z_PRCNAME='" & aModule & "' and T1.U_Z_ACTNAME ='" & aActivity & "' Order by  U_Z_DATE Desc"
            oRec1.DoQuery(strTimesheetQuery)
            If oRec1.RecordCount > 0 Then
                '   dblTotalTimesheet = oRec1.Fields.Item(0).Value
                aDate = oRec1.Fields.Item(0).Value
            End If
            strBudgetQuery = "Select T1.Docentry from [@Z_PRJ1] T0 Inner Join [@Z_HPRJ] T1 on T1.DocEntry=T0.DocEntry where T1.U_Z_PRJCODE='" & aProjectCode & "' and U_Z_MODNAME='" & aModule & "' and U_Z_ACTNAME='" & aActivity & "' and T0.U_Z_EMPID='" & aEmpID & "'"
            oRec.DoQuery(strBudgetQuery)
            If oRec.RecordCount > 0 Then
                '   strBudgetQuery = "Update   [@Z_PRJ1] set U_Z_STATUS='C', U_Z_CMPDATE='" & aDate.ToString("yyyy-MM-dd") & "'  where U_Z_STATUS<>'C' and DocEntry='" & oRec.Fields.Item(0).Value & "' and U_Z_MODNAME='" & aModule & "' and U_Z_ACTNAME='" & aActivity & "' and U_Z_EMPID='" & aEmpID & "'"
                strBudgetQuery = "Update   [@Z_PRJ1] set U_Z_STATUS='C', U_Z_CMPDATE='" & aDate.ToString("yyyy-MM-dd") & "'  where  DocEntry='" & oRec.Fields.Item(0).Value & "' and U_Z_MODNAME='" & aModule & "' and U_Z_ACTNAME='" & aActivity & "' and U_Z_EMPID='" & aEmpID & "'"
                oRec.DoQuery(strBudgetQuery)
            End If
...
#End Region
    ...
    Public Function getPriceforSupplier(ByVal aVendorCodea As String, ByVal aItemCode As String) As Double
        Dim dblPrice As Double = 0
        Dim oTest As SAPbobsCOM.Recordset
        Dim strSQL As String
        oTest = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        strSQL = "SELECT T0.[ListNum],CardCode,T1.[Price], T1.[Currency],T1.[ItemCode] FROM OCRD T0  left outer Join ITM1 T1 On T1.PriceList=T0.ListNum WHERE T0.[CardCode] ='" & aVendorCodea & "' and T1.ItemCode='" & aItemCode & "'"
        oTest.DoQuery(strSQL)
        dblPrice = oTest.Fields.Item(2).Value
        Return dblPrice
    ...
    Public Function createHRMainAuthorization() As Boolean
        ...
        mUserPermission.PermissionID = "PRJ"
        '//The Name value that will be displayed in the General Authorization Tree
        mUserPermission.Name = "Project Management Addon"
    ...
    Public Function addChildAuthorization(ByVal aChildID As String, ByVal aChildiDName As String, ByVal aorder As Integer, ByVal aFormType As String, ByVal aParentID As String, ByVal Permission As SAPbobsCOM.BoUPTOptions) As Boolean
        ...
        Dim ErrMsg As String
        Dim mUserPermission As SAPbobsCOM.UserPermissionTree
        If IsAuthorizationExits(aFormType, aChildID) = True Then
            Return True
        End If
        mUserPermission = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oUserPermissionTree)
    ...
    Public Sub AuthorizationCreation()
        addChildAuthorization("Setup", " Setup", 2, "", "PRJ", SAPbobsCOM.BoUPTOptions.bou_FullReadNone)
        addChildAuthorization("Trans", "Transactions", 2, "", "PRJ", SAPbobsCOM.BoUPTOptions.bou_FullReadNone)
        'Setup
        addChildAuthorization("PhaseMaster", "Project Phase Master", 3, frm_Module, "Setup", SAPbobsCOM.BoUPTOptions.bou_FullNone)
        addChildAuthorization("ActMaster", "Activity Master", 3, frm_Activity, "Setup", SAPbobsCOM.BoUPTOptions.bou_FullReadNone)
        addChildAuthorization("ExpMaster", "Expenses Master", 3, frm_Expances, "Setup", SAPbobsCOM.BoUPTOptions.bou_FullReadNone)
        addChildAuthorization("EmpCost", "Employee Costs", 3, frm_EmployeePostion, "Setup", SAPbobsCOM.BoUPTOptions.bou_FullReadNone)
        addChildAuthorization("LeaveType", "Leave Types Setup", 3, frm_LeaveType, "Setup", SAPbobsCOM.BoUPTOptions.bou_FullReadNone)
        addChildAuthorization("Login", "ESS Login credenials", 3, frm_LoginSetup, "Setup", SAPbobsCOM.BoUPTOptions.bou_FullReadNone)
        'Transactions

        'Project Setup
        addChildAuthorization("PrjSetup", "Project Setup", 3, "", "Trans", SAPbobsCOM.BoUPTOptions.bou_FullReadNone)
        addChildAuthorization("PrjReport", "Project Management Reports", 3, frm_PrjReports, "Trans", SAPbobsCOM.BoUPTOptions.bou_FullReadNone)
        'addChildAuthorization("Initilize", "Appraisals Initializations", 4, "frm_hr_IniAppraisal", "PrjSetup", SAPbobsCOM.BoUPTOptions.bou_FullNone)
        addChildAuthorization("Template", "Project Template", 4, frm_BudgetTemplate, "PrjSetup", SAPbobsCOM.BoUPTOptions.bou_FullReadNone)
        addChildAuthorization("Budget", "Budget Setup", 4, frm_BudgetEntry, "PrjSetup", SAPbobsCOM.BoUPTOptions.bou_FullReadNone)
        addChildAuthorization("Contract", "Contract Management", 4, frm_BudgetContract, "PrjSetup", SAPbobsCOM.BoUPTOptions.bou_FullReadNone)
        addChildAuthorization("BOQ", "BOQ Details", 4, frm_BOQDeatils, "PrjSetup", SAPbobsCOM.BoUPTOptions.bou_FullReadNone)
        addChildAuthorization("SCTime", "SubContractTimesheet", 4, frm_ContrctTimeSheet, "PrjSetup", SAPbobsCOM.BoUPTOptions.bou_FullNone)
    ...
    Public Function getLoggedonEmployee() As String
        Dim oTest As SAPbobsCOM.Recordset
        Dim intManagerid As Integer
        Dim strEmp As String = oApplication.Company.UserName
        oTest = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        oTest.DoQuery("SELECT *  FROM OHEM T0  INNER JOIN OUSR T1 ON T0.userId = T1.USERID WHERE T1.[USER_CODE] ='" & strEmp & "'")
        If oTest.RecordCount > 0 Then
            intManagerid = oTest.Fields.Item("empID").Value
            Return intManagerid.ToString
        Else
            Return "0"
        End If
        oTest.DoQuery("Select * from [@Z_Login] where U_EMPID='" & intManagerid & "'")
        If oTest.Fields.Item("U_SUPERUSER").Value = "Y" Then
            Return True
        Else
    ...
    Public Function CheckSuperUser(ByVal aCode As String) As String
        Dim oTest As SAPbobsCOM.Recordset
        Dim intManagerid As Integer
        Dim strEmp As String = oApplication.Company.UserSignature
        oTest = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        oTest.DoQuery("Select * from [@Z_Login] where U_EMPID='" & aCode & "'")
        If oTest.Fields.Item("U_SUPERUSER").Value = "Y" Then
            Return True
        Else
            Return False
    ...
    End Function
    Public Function IsAuthorizationExits(ByVal aFormUID As String, ByVal aChildID As String) As Boolean
        Dim oAuth As SAPbobsCOM.Recordset
        oAuth = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        Dim struserid As String
        struserid = oApplication.Company.UserName
...
#Region "Check Module/Activity"
    ...
    Public Function CheckProject_Contract(ByVal strproject As String, ByVal strContractID As String) As Boolean
        Dim oTe As SAPbobsCOM.Recordset
        Dim strQry As String
        oTe = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        strQry = "select U_Z_PRJCODE,U_Z_PRJNAME,T0.U_Z_STATUS  from [@Z_HPRJ] T0  inner Join [@Z_PRJ1] T1 on T1.DocEntry=T0.DocEntry where T1.U_Z_CNTID='" & strContractID & "' and T0.U_Z_PRJCODE='" & strproject & "' and T0.U_Z_Status='X'"
        oTe.DoQuery(strQry)
    ...
    Public Function CheckModule_Contract(ByVal strproject As String, ByVal strContractID As String, ByVal strPhase As String) As Boolean
        Dim oTe As SAPbobsCOM.Recordset
        Dim strQry As String
        oTe = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        If strPhase <> "" Then
            '   strQry = "select T1.U_Z_EMPCODE,T1.U_Z_EMPNAME,T1.U_Z_DESIGNATION  from [@Z_OPAT] T0 inner Join [@Z_PAT2] T1 on T1.DocEntry=T0.DocEntry where T0.DocEntry=" & strContractID & " and T1.U_Z_EMPCODE='" & strproject & "'"
            strQry = "select U_Z_PRJCODE,U_Z_PRJNAME,T0.U_Z_STATUS  from [@Z_HPRJ] T0  inner Join [@Z_PRJ1] T1 on T1.DocEntry=T0.DocEntry where T1.U_Z_CNTID='" & strContractID & "' and T0.U_Z_PRJCODE='" & strproject & "' and T1.U_Z_MODNAME='" & strPhase & "' and  T0.U_Z_Status='X' "
    ...
    Public Function CheckEMP_Contract(ByVal strEMP As String, ByVal strContractID As String) As Boolean
        Dim oTe As SAPbobsCOM.Recordset
        Dim strQry As String
        oTe = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        If strEMP <> "" Then
            strQry = "select T1.U_Z_EMPCODE,T1.U_Z_EMPNAME,T1.U_Z_DESIGNATION  from [@Z_OPAT] T0 inner Join [@Z_PAT2] T1 on T1.DocEntry=T0.DocEntry where T0.DocEntry=" & strContractID & " T1.U_Z_EMPCODE='" & strEMP & "'"
        Else
            strQry = "select T1.U_Z_EMPCODE,T1.U_Z_EMPNAME,T1.U_Z_DESIGNATION  from [@Z_OPAT] T0 inner Join [@Z_PAT2] T1 on T1.DocEntry=T0.DocEntry where T0.DocEntry=" & strContractID
    ...
    Public Function CheckModule_Activity(ByVal strproject As String, ByVal strTable As String, ByVal strValue As String, ByVal strField As String) As Boolean
        ...
        End If
    End Function
    Public Function CheckActivity(ByVal strproject As String, ByVal strTable As String, ByVal strValue As String, ByVal strvalue1 As String, ByVal strfield As String, ByVal strField1 As String, Optional ByVal aEmpId As String = "") As Boolean
        Dim oTe As SAPbobsCOM.Recordset
        Dim strQry As String
        oTe = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        If aEmpId = "" Then
        ...
        Else
            strQry = "Select * from " & strTable & " where U_Z_STATUS='I' and  U_Z_EMPID='" & aEmpId & "'and " & strfield & "='" & strValue.ToUpper.Replace("'", "''") & "' and " & strField1 & "='" & strvalue1.Replace("'", "''") & "' and docentry=(Select DocEntry from [@Z_HPRJ] where U_Z_PRJCODE='" & strproject.Replace("'", "''") & "')"
...
#Region "Get LocalcurrencyAmount"
    Public Sub updatelocalamount(ByVal aCode As String)
        ...
        Dim dblExchangerate, dblAmount, dblExpAmount As Double
        Dim dblDate As Date
        ...
        For intRow As Integer = 0 To oAmtRS.RecordCount - 1
            ...
            Else
                ...
                oExchangeRS.DoQuery(strsql)
            End If
            If systemcurrency <> LocalCurrency Then
                strsql = "Select * from  [@Z_EXP1]  where code='" & strcode & "'"
                oExchangeRS.DoQuery(strsql)
                dblAmount = getDocumentQuantity(oExchangeRS.Fields.Item("U_Z_LocAmt").Value)
                strsql = "select isnull(rate,1) from [ortt] where currency='" & systemcurrency & "'and convert(varchar(10),ratedate,105)='" & dblDate.ToString("dd-MM-yyyy") & "'"
                oExchangeRS = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                oExchangeRS.DoQuery(strsql)
                If oExchangeRS.RecordCount > 0 Then
                    dblAmount = dblAmount / oExchangeRS.Fields.Item(0).Value
                Else
                    dblAmount = dblAmount / 1 'oExchangeRS.Fields.Item(0).Value
                End If

                dblAmount = Math.Round(dblAmount, 3)
                stramount = systemcurrency & " " & dblAmount.ToString
                strsql = "Update [@Z_EXP1] set U_Z_SysAmt='" & stramount & "' where code='" & strcode & "'"
                oExchangeRS.DoQuery(strsql)
            Else
                oExchangeRS = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                dblAmount = dblExpAmount
                stramount = LocalCurrency & " " & dblAmount.ToString
                strsql = "Update [@Z_EXP1] set U_Z_SysAmt=U_Z_LocAmt where code='" & strcode & "'"
                oExchangeRS.DoQuery(strsql)
            End If
            oExchangeRS.DoQuery(strsql)
            oAmtRS.MoveNext()
    ...
    Public Sub updatelocalamount_Approval(ByVal aCode As String)
        Dim oAmtRS, oExchangeRS As SAPbobsCOM.Recordset
        Dim strCurrency, strsql, strcode, stramount As String
        ...
        oAmtRS = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        'oAmtRS.DoQuery("SELECT T0.[U_Z_DOCDATE], T1.[Code], T1.[U_Z_CURRENCY], T1.[U_Z_AMOUNT] FROM [dbo].[@Z_OEXP]  T0  inner join  [dbo].[@Z_EXP1]  T1 on T1.[U_Z_REFCODE]=T0.Code where T0.Code='" & aCode & "'")
        oAmtRS.DoQuery("SELECT T0.[U_Z_DOCDATE], T1.[Code], T1.[U_Z_CURRENCY], T1.[U_Z_AMOUNT] FROM [dbo].[@Z_OEXP]  T0  inner join  [dbo].[@Z_EXP1]  T1 on T1.[U_Z_REFCODE]=T0.Code where T1.[Code]='" & aCode & "'")
        Dim dblExpAmount As Double
        For intRow As Integer = 0 To oAmtRS.RecordCount - 1
            strcode = oAmtRS.Fields.Item("Code").Value
            dblAmount = oAmtRS.Fields.Item("U_Z_AMOUNT").Value
            dblExpAmount = dblAmount
...
#End Region
    ...
    Public Function createPurchaseRequest(ByVal aCode As String) As Boolean
        ...
        If oRec.RecordCount > 0 Then
            ...
            strSQL = "Select ""U_Z_VENDOR"",Count(*) from ""@Z_PRJ2"" where  isnull(""U_Z_DOCENTRY"",0)<=0 and isnull(""U_Z_PR"",'N')='Y' and ""U_Z_BOQREF""='" & aCode & "' group by ""U_Z_VENDOR"""
            oRec.DoQuery(strSQL)
            For intRow As Integer = 0 To oRec.RecordCount - 1
                ...
                If oUser.GetByKey(oApplication.Company.UserSignature) Then
                    ...
                    If oRec.Fields.Item(0).Value <> "" Then
                        s = "Select * from ""@Z_PRJ2"" where isnull(""U_Z_DOCENTRY"",0)<=0 and  ""U_Z_BOQREF""='" & aCode & "' and ""U_Z_VENDOR""='" & oRec.Fields.Item(0).Value & "' and ""U_Z_PR""='Y'"
                    Else
                        s = "Select * from ""@Z_PRJ2"" where isnull(""U_Z_DOCENTRY"",0)<=0 and  ""U_Z_BOQREF""='" & aCode & "' and ""U_Z_PR""='Y'"
                    ...
                    For intLoop As Integer = 0 To oRec1.RecordCount - 1
                        ...
                        If oRec.Fields.Item("U_Z_VENDOR").Value <> "" Then
