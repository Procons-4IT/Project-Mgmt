Public Class clsChangePassword
    ...
    End Sub
    Public Sub LoadForm()
        oForm = oApplication.Utilities.LoadForm("frm_ChangePassword.xml", "frm_ChngPwd")
        ...
        oTes.DoQuery(st)
        oApplication.Utilities.setEdittextvalue(oForm, "4", oTes.Fields.Item(1).Value)
        'AddChooseFromList(oForm)
    End Sub
    Private Function LogonLoginDetails(ByVal aForm As SAPbouiCOM.Form) As Boolean
        Dim otemp As SAPbobsCOM.Recordset
        otemp = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        Dim oldPas, newpas, newpas1, existingPassword As String
        oldPas = oApplication.Utilities.getEdittextvalue(oForm, "6")
        newpas = oApplication.Utilities.getEdittextvalue(oForm, "8")
        newpas1 = oApplication.Utilities.getEdittextvalue(oForm, "10")
        If oldPas = "" Then
            oApplication.Utilities.Message("Login password is missing...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Return False

        End If
        If newpas = "" Then
            oApplication.Utilities.Message("New password is missing...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Return False

        End If
        If newpas1 = "" Then
            oApplication.Utilities.Message("Confirm New password is missing...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Return False
        End If
      
        otemp.DoQuery("Select U_UID,U_Pwd, U_EmpiD,isnull(U_Approver,'N') from [@Z_Login] where U_UID='" & oApplication.Company.UserName & "'")
        If otemp.RecordCount > 0 Then
            existingPassword = otemp.Fields.Item(1).Value
        Else
            Return False
        End If
        If existingPassword.ToUpper <> oldPas.ToUpper Then
            oApplication.Utilities.Message("Invalid Login  ..", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Return False
        End If

        If newpas.ToUpper = oldPas.ToUpper Then
            oApplication.Utilities.Message("New password should be different then the Old password", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Return False
        End If

        If newpas.ToUpper <> newpas1.ToUpper Then
            oApplication.Utilities.Message("Confirm New password does not match with New password", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
            Return False
        End If

        otemp = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        otemp.DoQuery("Update ""@Z_Login"" set U_Pwd ='" & newpas & "' where U_UID='" & oApplication.Company.UserName & "'")
        oApplication.Utilities.Message("Password changed successfully", SAPbouiCOM.BoStatusBarMessageType.smt_Success)
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_ChangePassword Then
                        ...
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                                ...
                                If pVal.ItemUID = "1" And oForm.Mode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE Then
                                    If LogonLoginDetails(oForm) = False Then
                                        BubbleEvent = False
                                        Exit Sub
                                    Else
                                        oForm.Close()
