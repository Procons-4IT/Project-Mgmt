    ...
    Private Sub databind(ByVal aForm As SAPbouiCOM.Form)
        Try
            ...
            If strFromEMP <> "" And strToEMP <> "" Then
                ...
                strExmEmpCondition = " Convert(Decimal,T1.U_Z_EMPID) between " & CDbl(strFromEMP) & " and " & CDbl(strToEMP)
            ElseIf strFromEMP <> "" And strToEMP = "" Then
                strEMPCondition = " Convert(Decimal,U_Z_EMPCODE) >= " & CDbl(strFromEMP)
                strExmEmpCondition = " Convert(Decimal,T1.U_Z_EMPID) >= " & CDbl(strFromEMP)
            ElseIf strFromEMP = "" And strToEMP <> "" Then
                strEMPCondition = " Convert(Decimal,U_Z_EMPCODE) <= " & CDbl(strToEMP)
                strExmEmpCondition = " Convert(Decimal,T1.U_Z_EMPID) <= " & CDbl(strToEMP)
            Else
                strEMPCondition = " 1=1"
            ...
            Dim strTimeAttendance, strCondition, strGroupcondition, st1, st2, strJEPostingCondition, strLeaveCondition, strExpPostCondition As String
            ...
            If (strreporttype.ToUpper = "EXP") Then
                ...
                strString = "SELECT T1.[U_Z_EMPID], T1.[U_Z_EMPNAME], convert(varchar(10),T0.[U_Z_DOCDATE],105), T1.[U_Z_EXPNAME], T1.[U_Z_EXPTYPE], T1.[U_Z_PRJCODE],T1.[U_Z_PRJNAME],"
                '  strString = strString & " T1.U_Z_MODNAME,T1.U_Z_ACTNAME, T1.[U_Z_CURRENCY], T1.[U_Z_AMOUNT], T1.[U_Z_LocAmt], (replace(T1.[U_Z_LocAmt],substring(T1.[U_Z_LocAmt],0,4),'')), T1.[U_Z_REFCODE], T1.[U_Z_REMARKS], case T1.[U_Z_APPROVED] when 'A' then 'Approved'  when 'P' then 'Pending'  else 'Declined' end ,T1.[U_Z_DocNum] FROM [dbo].[@Z_OEXP]  T0 inner join  [dbo].[@Z_EXP1]  T1  on T0.Code=T1.U_Z_REFCODE  where " & strCondition
                strString = strString & " T1.U_Z_MODNAME,T1.U_Z_ACTNAME, T1.[U_Z_CURRENCY], T1.[U_Z_AMOUNT], T1.[U_Z_LocAmt], (replace(T1.[U_Z_LocAmt],substring(T1.[U_Z_LocAmt],0,4),'')), T1.[U_Z_REFCODE], T1.[U_Z_REMARKS], case T1.[U_Z_APPROVED] when 'A' then 'Approved'  when 'P' then 'Pending'  else 'Declined' end ,T1.[U_Z_DocNum] FROM [dbo].[@Z_OEXP]  T0 inner join  [dbo].[@Z_EXP1]  T1  on T0.Code=T1.U_Z_REFCODE  where T1.U_Z_EMPID in (" & strEMPIDS & ") and " & strExpPostCondition
                strString1 = ""
                strString2 = ""
            ...
            ElseIf strreporttype.ToUpper = "TA" Then
                strString = "SELECT  T0.[U_Z_EMPCODE], T0.[U_Z_EMPNAME], convert(varchar(10),T0.[U_Z_DOCDATE],105) 'Act.Date', ' ' 'U_Z_PRJCODE' ,' ' 'U_Z_PRJNAME' ,' ' 'U_Z_PRCNAME' ,' ' 'U_Z_ACTNAME', T0.[U_Z_TYPE], T0.[U_Z_FROMDATE]  ,T0.[U_Z_TODATE], T0.[U_Z_DAYS] 'Days/Hours', T0.[U_Z_REASON] 'Reason /Remarks' ,  case T0.[U_Z_APPROVED] when 'A' then 'Approved' when 'P' then 'Pending' else 'Declined' end 'STATUS' FROM [dbo].[@Z_OLEV]  T0  where T0.[U_Z_EMPCODE] in (" & strEMPIDS & ") and " & strTimeAttendance
                strString = strString & "   union all"
                strString = strString & " SELECT T0.[U_Z_EMPCODE], T0.[U_Z_EMPNAME], convert(varchar(10),T1.[U_Z_DATE],105) 'Act.Date', T1.[U_Z_PRJCODE],T1.[U_Z_PRJNAME], T1.[U_Z_PRCNAME], T1.[U_Z_ACTNAME],  case isnull(T1.[U_Z_TYPE],'I') when 'I' then 'Item' else 'Resource' end 'U_Z_TYPE',T1.[U_Z_DATE] 'U_Z_FROMDATE',T1.[U_Z_DATE] 'U_Z_TODATE'  ,  T1.[U_Z_HOURS] 'Days/Hours',  T1.[U_Z_REMARKS] 'Reason /Remarks', case T1.[U_Z_APPROVED] when 'A' then 'Approved' when 'P' then 'Pending' else 'Declined' end 'STATUS' FROM [dbo].[@Z_OTIM]  T0  inner join  [dbo].[@Z_TIM1]  T1 on T0.Code=T1.U_Z_REFCODE   where T0.[U_Z_EMPCODE] in (" & strEMPIDS & ") and " & strTimeAttendance
            ...
            ElseIf strreporttype.ToUpper = "PRJM" Then
                ...
                '   strString = strString & "   T2.U_Z_ESTCOST(-T2.U_Z_ESTCOST) 'RelAmt' from [@Z_PRJ1] T1 inner Join [@Z_HPRJ] T0 on T0.DOcEntry=T1.DocEntry left outer Join [@Z_PRJ2] T2 on T2.U_Z_BOQREF=T1.U_Z_BOQ "
                strString = strString & "   T2.U_Z_ESTCOST-T2.U_Z_ESTCOST 'RelCost' from [@Z_PRJ1] T1 inner Join [@Z_HPRJ] T0 on T0.DOcEntry=T1.DocEntry left outer Join [@Z_PRJ2] T2 on T2.U_Z_BOQREF=T1.U_Z_BOQ where " & projectMaterialcond
            ElseIf strreporttype.ToUpper = "PRJS" Then
                '  strString = " SELECT T0.[U_Z_PRJCODE],T0.[U_Z_PRJName], T1.[U_Z_MODNAME], T1.[U_Z_ACTNAME], T1.[U_Z_EmpID],T1.[U_Z_Position],  T1.[U_Z_FromDate],T1.[U_Z_ToDate],T1.[U_Z_HOURS],T1.[U_Z_HOURS],T1.[U_Z_HOURS] ,T1.[U_Z_AMOUNT],T1.[U_Z_AMOUNT],T1.[U_Z_AMOUNT] FROM [dbo].[@Z_HPRJ]  T0  inner join  [dbo].[@Z_PRJ1]  T1 on T0.DocEntry=T1.DocEntry where " & strProjectCondition
                strString = " T1.U_Z_Quantity 'Qty',0.00 'Percentage', case T1.U_Z_Status when 'I' then 'In Process' when 'C' then 'Completed' else 'Pending' end 'Status',T1.U_Z_CMPDATE 'CmpDate'"
                ...
                ' strString = st5 & " union all  SELECT T0.[U_Z_PRJCODE] 'Project',T0.[U_Z_PRJName] 'ProjectName', T1.[U_Z_MODNAME] 'Business', T1.[U_Z_ACTNAME] 'Activity', T1.[U_Z_EmpID] 'EmpID',T1.[U_Z_Position] 'Position',  T1.[U_Z_FromDate] 'FromDate',T1.[U_Z_ToDate] 'EndDate',T1.[U_Z_HOURS] 'EstimatedHours',T1.[U_Z_HOURS] 'ActualHours',T1.[U_Z_HOURS] 'VarianceHours' , " & strString & ",T1.[U_Z_AMOUNT] 'EstimatedCost',T1.[U_Z_Quantity]-T1.[U_Z_Quantity] 'MatrialOrdered',T1.[U_Z_Quantity]-T1.[U_Z_Quantity] 'MaterialReceived',T1.[U_Z_Quantity]-T1.[U_Z_Quantity] 'OrderPending',T1.[U_Z_Quantity]-T1.[U_Z_Quantity] 'MaterialPending', T1.[U_Z_Quantity]-T1.[U_Z_Quantity] 'Cost Variance' FROM [dbo].[@Z_HPRJ]  T0  inner join  [dbo].[@Z_PRJ1]  T1 on T0.DocEntry=T1.DocEntry where " & strProjectCondition
            ...
            ElseIf strreporttype.ToUpper = "POSTING" Then
                ...
                strString = strString & " T1.[U_Z_CURRENCY], T1.[U_Z_AMOUNT], T1.[U_Z_LocAmt], (replace(T1.[U_Z_LocAmt],T1.[U_Z_CURRENCY],'')), T1.[U_Z_REFCODE], T1.[U_Z_REMARKS], 'N' 'Select' ,T1.Code FROM [dbo].[@Z_OEXP]  T0 inner join  [dbo].[@Z_EXP1]  T1  on T0.Code=T1.U_Z_REFCODE  where T1.[U_Z_APPROVED]='A' and isnull(T1.[U_Z_DocNum],'')='' and " & strExpPostCondition
