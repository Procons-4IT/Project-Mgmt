...
#Region "Add Choose From List"
    ...
    Private Sub AddChooseFromList_Load(ByVal objForm As SAPbouiCOM.Form)
        Try
        ...
        For intRow As Integer = 1 To oMatrix.RowCount
            ...
            If strCode <> "" Then
                ...
                If CDbl(strcode1) <= 0 Then
                    ...
                    Return False
                End If
                strcode1 = oApplication.Utilities.getMatrixValues(oMatrix, "V_0", intRow)
                'If strcode1 <> "" Then
                '    Dim dtdate As Date = oApplication.Utilities.GetDateTimeValue(strcode1)
                '    Dim strQuery As String
                '    strQuery = "Select * from [@Z_PRJ1] T0 inner Join [@Z_HPRJ] T1 on T1.DocEntry=T0.DocEntry where T0.U_Z_EMPID='" & strEmp & "' and  T1.U_Z_PRJCODE='" & strProject & "' and T0.U_Z_MODNAME='" & strPhase & "' and T0.U_Z_ACTNAME='" & strActivity & "' and '" & dtdate.ToString("yyyy-MM-dd") & "' between T0.U_Z_FROMDATE and T0.U_Z_TODATE"
                '    Dim oRec12 As SAPbobsCOM.Recordset
                '    oRec12 = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                '    oRec12.DoQuery(strQuery)
                '    If oRec12.RecordCount <= 0 Then
                '        oApplication.Utilities.Message("Time sheet Date should be between budgeted from and end date : ", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                '        oMatrix.Columns.Item("V_0").Cells.Item(intRow).Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                '        Return False
                '    End If
                ...
                If strPrjCode <> "" Then
                    ...
                    If strDocref <> "" Then
                        If strLineCode = "" Then
                            ...
                            strProject = oApplication.Utilities.getMatrixValues(oBPGrid, "V_1", intLoop)
                            strBusiness = oApplication.Utilities.getMatrixValues(oBPGrid, "V_3", intLoop)
                            strActivtiy = oApplication.Utilities.getMatrixValues(oBPGrid, "V_4", intLoop)
                            ...
                            strsql = "Select * from [@Z_PRJ1] T0 inner Join [@Z_HPRJ] T1 on T1.DocEntry=T0.DocEntry and T0.U_Z_EMPID='" & oApplication.Utilities.getEdittextvalue(aform, "4") & "' where U_Z_MODNAME='" & strBusiness & "' and U_Z_ActName='" & strActivtiy & "' and  T1.U_Z_PRJCODE='" & strProject & "'"
                            oTe.DoQuery(strsql)
                        ...
                        Else
                            If ousertable.GetByKey(strLineCode) Then
                                ...
                                Else
                                    ...
                                    Dim oTest12 As SAPbobsCOM.Recordset
                                    oTest12 = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                                    strCode1 = strLineCode
                                    strQuery1 = "Select *  from [@Z_OTIM]  T0 inner Join [@Z_TIM1] T1  on T1.U_Z_REFCODE=T0.Code  where T1.Code='" & strCode1 & "'"
                                    oTest12.DoQuery(strQuery1)
                                    If oTest12.RecordCount > 0 Then
                                        oApplication.Utilities.ClosingProjectActivity(oTest12.Fields.Item("U_Z_EMPCODE").Value, oTest12.Fields.Item("U_Z_PRJCODE").Value, oTest12.Fields.Item("U_Z_PRCNAME").Value, oTest12.Fields.Item("U_Z_ACTNAME").Value, oTest12.Fields.Item("U_Z_DATE").Value)
                                    End If
                        ...
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                                ...
                                ElseIf pVal.ItemUID = "21" Then
                                    If oApplication.SBO_Application.MessageBox("Do you want to delete selected Time sheet entry?", , "Yes", "N0") = 2 Then
                                        Exit Sub
                                    End If
                                    deleterow(oForm)
                                End If
                            Case SAPbouiCOM.BoEventTypes.et_KEY_DOWN
                                ...
                                If pVal.ItemUID = "12" And pVal.ColUID = "V_3" And pVal.CharPressed = 9 Then
                                    ...
                                    Else
                                        ...
                                        strsql = "Select * from [@Z_PRJ1]" & " where U_Z_STATUS='I' and  U_Z_MODNAME='" & strBusiness & "' and U_Z_ActName='" & strActivtiy & "' and docentry=(Select DocEntry from [@Z_HPRJ] where U_Z_PRJCODE='" & strProject & "')"
                                        oTe.DoQuery(strsql)
                                        If oTe.RecordCount > 0 Then
                                            oForm.Freeze(True)
                                ...
                                If pVal.ItemUID = "12" And pVal.ColUID = "V_4" And pVal.CharPressed = 9 Then
                                    ...
                                    If oApplication.Utilities.CheckActivity(strwhs, "[@Z_PRJ1]", strGirdValue, strProcess, "U_Z_ACTNAME", "U_Z_MODNAME", oApplication.Utilities.getEdittextvalue(oForm, "4")) = False Then
