        ...
        Try
            ...
            If 1 = 1 Then
                ...
                If boqref <> "" Then
                    ...
                    strSQL = "SELECT T0.[Code], T0.[Name], T0.[U_Z_PRJCODE], T0.[U_Z_PRJNAME], T0.[U_Z_MODNAME], T0.[U_Z_ACTNAME], T0.[U_Z_BOQREF], T0.[U_Z_STATUS], T0.[U_Z_ITEMCODE], T0.[U_Z_ITEMNAME], T0.[U_Z_UOM],  T0.[U_Z_VENDOR], T0.[U_Z_VENDORNAME],T0.U_Z_UNITPRICE ,T0.[U_Z_REQQTY], T0.[U_Z_ESTCOST], T0.[U_Z_REQDATE], T0.[U_Z_COMMENTS], T0.[U_Z_DOCENTRY], T0.[U_Z_DOCNUM], T0.[U_Z_PR], T0.[U_Z_CNTID], T0.[U_Z_POSITION] FROM [dbo].[@Z_PRJ2]  T0"
                    strSQL = strSQL & " where U_Z_BOQRef='" & aCode & "'"

                Else
                    boqref = stCode
                    strSQL = "Select * from [@Z_PRJ2] where U_Z_BOQRef='" & boqref & "'"
                    strSQL = "SELECT T0.[Code], T0.[Name], T0.[U_Z_PRJCODE], T0.[U_Z_PRJNAME], T0.[U_Z_MODNAME], T0.[U_Z_ACTNAME], T0.[U_Z_BOQREF], T0.[U_Z_STATUS], T0.[U_Z_ITEMCODE], T0.[U_Z_ITEMNAME], T0.[U_Z_UOM],  T0.[U_Z_VENDOR], T0.[U_Z_VENDORNAME],T0.U_Z_UNITPRICE ,T0.[U_Z_REQQTY], T0.[U_Z_ESTCOST], T0.[U_Z_REQDATE],  T0.[U_Z_COMMENTS], T0.[U_Z_DOCENTRY], T0.[U_Z_DOCNUM], T0.[U_Z_PR], T0.[U_Z_CNTID], T0.[U_Z_POSITION] FROM [dbo].[@Z_PRJ2]  T0"
                    strSQL = strSQL & " where U_Z_BOQRef='" & boqref & "'"
                End If
                ' dtTemp.ExecuteQuery(strSQL)
                ...
                Dim st1 As String = "Select * from [@Z_HPRJ] where U_Z_PrjCode='" & oApplication.Utilities.getEdittextvalue(aForm, "etFind") & "'"
                oTemp.DoQuery(st1)
                oApplication.Utilities.setEdittextvalue(aForm, "21", oTemp.Fields.Item("DocEntry").Value)
                Dim intDocentry As Integer = oTemp.Fields.Item("DocEntry").Value
                ' Dim st As String = "Update [@Z_PRJ1] set U_Z_BOQ='" & boqref & "' where lineid=" & oApplication.Utilities.getEdittextvalue(aForm, "19") & " and DocEntry=" & oApplication.Utilities.getEdittextvalue(aForm, "21")
                Dim st As String = "Update [@Z_PRJ1] set U_Z_BOQ='" & boqref & "' where lineid=" & oApplication.Utilities.getEdittextvalue(aForm, "19") & " and DocEntry=" & intDocentry
                oTemp.DoQuery(st)
            End If
            oForm = objform
            FormatGrid(objGrid)
...
#End Region
    ...
    End Sub
    Private Sub AssignLineNo(ByVal agrid As SAPbouiCOM.Grid, ByVal aform As SAPbouiCOM.Form)
        Try
            aform.Freeze(True)
    ...
    Private Sub FormatGrid(ByVal aGrid As SAPbouiCOM.Grid)
        ...
        aGrid.Columns.Item("U_Z_UNITPRICE").TitleObject.Caption = "Unit Price"
        ...
        For intRow As Integer = 0 To oBPGrid.DataTable.Rows.Count - 1
            If oBPGrid.DataTable.GetValue("U_Z_ITEMCODE", intRow) <> "" Then
                ...
                If strCode = "" Then
                    ...
                    ousertable.UserFields.Fields.Item("U_Z_UNITPRICE").Value = oBPGrid.DataTable.GetValue("U_Z_UNITPRICE", intRow)
    ...
    Private Function validation(ByVal aGrid As SAPbouiCOM.Grid) As Boolean
        For intRow As Integer = 0 To aGrid.DataTable.Rows.Count - 1
            If aGrid.DataTable.GetValue("U_Z_ITEMCODE", intRow) <> "" Then
                If aGrid.DataTable.GetValue("U_Z_DOCENTRY", intRow) = 0 Then
                ...
                Select Case pVal.BeforeAction
                                ...
                                If pVal.ItemUID = "mtchoose" Then
                                    ...
                                    Try
                                        If objGrid.DataTable.GetValue("U_Z_DOCNUM", pVal.Row) <> 0 Then
                                            oApplication.Utilities.Message("Purchase Request already created for this entry . You can not modify any data", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                                            BubbleEvent = False
                                            Exit Sub
                                        End If
                                    Catch ex As Exception
                    ...
                    Case False
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                                ...
                                Select Case pVal.ItemUID
                                    Case "30"
                                        ...
                                        Dim objChoose As New clsBOMCopy
                                        ...
                                        If 1 = 1 Then
                                            ...
                                            oApplication.Utilities.LoadForm(xml_BOMWizard, frm_BOMWizard)
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_KEY_DOWN
                                oForm = oApplication.SBO_Application.Forms.Item(FormUID)
                                If pVal.ItemUID = "mtchoose" And (pVal.ColUID = "U_Z_UNITPRICE" Or pVal.ColUID = "U_Z_REQQTY") And pVal.CharPressed = 9 Then
                                    objGrid = oForm.Items.Item("mtchoose").Specific
                                    objGrid.DataTable.SetValue("U_Z_ESTCOST", pVal.Row, objGrid.DataTable.GetValue("U_Z_UNITPRICE", pVal.Row) * objGrid.DataTable.GetValue("U_Z_REQQTY", pVal.Row))
                                ...
                                Try
                                    ...
                                    If (oCFLEvento.BeforeAction = False) Then
                                        ...
                                        If pVal.ItemUID = "mtchoose" And pVal.ColUID = "U_Z_ITEMCODE" Then
                                            ...
                                            objGrid.DataTable.SetValue("U_Z_VENDOR", pVal.Row, oDataTable.GetValue("CardCode", 0))
                                            Dim ots As SAPbobsCOM.Recordset
                                            ots = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                                            If oDataTable.GetValue("CardCode", 0) <> "" Then
                                                ots.DoQuery("Select CardName from OCRD where CardCode='" & oDataTable.GetValue("CardCode", 0) & "'")
                                                objGrid.DataTable.SetValue("U_Z_VENDORNAME", pVal.Row, ots.Fields.Item(0).Value)
                                            Else
                                                objGrid.DataTable.SetValue("U_Z_VENDORNAME", pVal.Row, "")

                                            End If
                                            Dim dblPrice As Double = oApplication.Utilities.getPriceforSupplier(oDataTable.GetValue("CardCode", 0), val)
                                            objGrid.DataTable.SetValue("U_Z_UNITPRICE", pVal.Row, dblPrice)
                                        ...
                                        If pVal.ItemUID = "etFind" Then
                                            ...
                                            Dim val2 As String = oDataTable.GetValue("DocEntry", 0)
                                            Try
                                                oApplication.Utilities.setEdittextvalue(oForm, "21", val2)
                                        ...
                                        If pVal.ItemUID = "mtchoose" And pVal.ColUID = "U_Z_VENDOR" Then
                                            ...
                                            Dim dblPrice As Double = oApplication.Utilities.getPriceforSupplier(oDataTable.GetValue("CardCode", 0), objGrid.DataTable.GetValue("U_Z_ITEMCODE", pVal.Row))
                                            objGrid.DataTable.SetValue("U_Z_UNITPRICE", pVal.Row, dblPrice)
                                        End If
                                        oForm.Freeze(False)
                                    End If
